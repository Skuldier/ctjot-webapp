...
TRUE_RANDOM_CATEGORIES: tuple[tuple[str, str, str, bool, int, str], ...] = (
    ...
    # Default weight is 50 (matches AP convention for weight-style sliders).
    # cjot-beta normalizes weights so the absolute number doesn't matter --
    # only ratios do -- but starting all at 50 makes the slider position
    # feel intuitive ("middle of the range") to AP-familiar users.
    # --- Quest categories (random-pull) ---
    ("quest-free",    "Non-Gated Quest",  "quest_free",    True,  50,
     "Pulls from non-gated quests (Cathedral, Heckran's Cave, Denadoro, Zenan Bridge) -- no key-item gate required."),
    ("quest-gated",   "Gated Quest",      "quest_gated",   True,  50,
     "Pulls from gated quest objectives (story-driven content the rando placed behind a key-item gate)."),
    ("quest-late",    "Hard Quest",       "quest_late",    True,  50,
     "Pulls from late-game quests (typically high-difficulty endgame content)."),
    ("quest-go",      "Go Mode Quest",    "quest_go",      True,  50,
     "Pulls from quests in go-mode dungeons (the path to Lavos for the seed)."),
    # --- Recruit categories ---
    ("recruit",       "Gated Recruit",    "recruit_gated", True,  50,
     "Pulls from character-recruit objectives (recruit X from gated location)."),
    ("recruit-3",     "Recruit 3 Characters (Total 5)",  "recruit_3", False, 50,
     "Specific objective: recruit 3 of the gated characters (total party of 5)."),
    ("recruit-4",     "Recruit 4 Characters (Total 6)",  "recruit_4", False, 50,
     "Specific objective: recruit 4 of the gated characters (total party of 6)."),
    ("recruit-5",     "Recruit 5 Characters (Total 7)",  "recruit_5", False, 50,
     "Specific objective: recruit all 5 gated characters (total party of 7)."),
    # --- Boss categories (random-pull) ---
    ("boss-any",      "Any Boss",         "boss_any",      True,  50,
     "Pulls from any boss across the world, including go-mode dungeons."),
    ("boss-go",       "Go Mode Boss",     "boss_go",       True,  50,
     "Pulls from bosses inside go-mode dungeons only."),
    ("boss-nogo",     "Non-Go Boss",      "boss_nogo",     True,  50,
    ...
    ("collect-3-rocks", "Collect 3 Rocks", "collect_3_rocks", False, 50,
     "Specific objective: collect any 3 of the 5 rocks. Requires Rocksanity for the rocks to be findable."),
    ("collect-4-rocks", "Collect 4 Rocks", "collect_4_rocks", False, 50,
